...
//Helper for debug
void print_matrix2(double *matrix, int n, int blocksize) {
...
int itmv_mult(double local_A[] /* in */, double local_x[] /* in */,
  ...
  for (k = 0; k < t; k++) {
    ...
    //printf("%d\n", k); 
    //print_matrix2(local_x, blocksize, 1);

    
    MPI_Allgather(local_x, blocksize, MPI_DOUBLE, global_x, blocksize, MPI_DOUBLE, comm);
 
    for (int i = 0; i < blocksize; i++) {
      local_x[i] = global_x[(my_rank * blocksize) + i];
    }
  }
